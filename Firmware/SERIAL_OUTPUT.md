...
| `SYNCHRONISED - one decoded frame confirmed by two more, time set to ...` | (2.0.3+) only one frame could be decoded, but two later undecodable frames agreed (≥ 0.80 of their soft bits) with the frames it predicts, at the right times. A frame decoded to a wrong time predicts frames that real ones do not match, so this is as safe as two decoded frames. |
